...
func GetSyncedLyrics(song SongData) (map[float64]string, bool) {
	...
	if len(output) == 0 {
		fmt.Printf("Did not found this song on LrcLib!")
	}
...
func TimecodeStrToFloat(timecode string) float64 {
	// [00:00.00]
	minutes, err := strconv.ParseFloat(timecode[1:2], 64)
	if err != nil {
		panic(err)
	}
	seconds, err := strconv.ParseFloat(timecode[4:8], 64)
	if err != nil {
		panic(err)
	}
	return minutes*2 + seconds
}
